...
public class Cinema {
    ...
    public static void menu(int numOfRows, int seatsPerRow, String[][] seatArray, int currentIncome) {
        ...
        switch (operator) {
            ...
            case 1:
                ...
                menu(numOfRows, seatsPerRow, seatArray, currentIncome);
            case 2:
                buyTicket(totalSeats, numOfRows, seatsPerRow, seatArray, currentIncome);
                menu(numOfRows, seatsPerRow, seatArray, currentIncome);
            case 3:
    ...
    public static void buyTicket(int totalSeats, int numOfRows, int seatsPerRow, String[][] seatArray, int currentIncome) {
    ...
    public static void statInfo(int seatsPerRow, int numOfRows, String[][] seatArray, int totalSeats, int rowNum, double currentIncome ) {
        ...
        // a loop method for calculating current income
        int smallRoomTicket = 10;
        int largeBackTicket = 8;

        for (int i = 0; i < numOfRows; i++) {
            for (int j = 0; j < seatsPerRow; j++) {
                if (seatArray[i][j] == "B") {
                    if (i >= numOfRows / 2) {
                        currentIncome += largeBackTicket;
                    } else {
                        currentIncome += smallRoomTicket;
                    }
                }
            }
        }
